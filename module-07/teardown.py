"""teardown.py — remove Module 7's MCP Lambda + role, KEEP what Module 8 needs.

Module 7 of AWS GenAI Pro Mastery. Idempotent and verbose, and TESTED — one tested
teardown per setup (decision B5).

What this DELETES BY DEFAULT (Module 7's idle-billed / standing resources):
  1. the CloudCart MCP server Lambda `relay-mcp-server` and its Function URL config;
  2. the MCP Lambda's IAM execution role `relay-mcp-lambda-role` + its inline policy;
...
Run it:
    uv run python teardown.py                  # delete MCP Lambda + role; KEEP tables + KB
...
def assert_clean(*, drop_tables: bool, drop_kb: bool, drop_vectors: bool,
    ...
    print("\nIdle-billed resources from Module 7: NONE remain.")
    ...
    if drop_kb:
        ...
        print(f"  - Knowledge Base '{config.RELAY_KB_NAME}' + role: KEPT — search_kb "
              "retrieves from it and Module 8 needs it (~$0 idle). Drop with --delete-kb.")
    if drop_vectors:
    ...
    print("\nTo rebuild for Module 8: uv run python setup.py "
          "(recreates the MCP Lambda; tables/KB kept).")


_FLAGS = ("--delete-tables", "--delete-kb", "--delete-vectors")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if leftover:
        print(f"Unknown argument(s): {' '.join(leftover)}\n"
              ...
              "[--delete-tables] [--delete-kb] [--delete-vectors]",
              file=sys.stderr)
        return 1

    print("Tearing down Module 7 (idempotent).\n")
    ...
    try:
        print("CloudCart MCP server (AWS Lambda + Function URL):")
