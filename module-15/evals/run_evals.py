...
MODULE 14 ADDITION (by addition): with --emit-metrics, the run also publishes its aggregate
grounding to CloudWatch as the EvalGrounding metric (observability.metrics) — the golden set
used as a PRODUCTION CANARY (skill 4.3.6). The `relay-ops` grounding<0.8 alarm watches it (the
SAME 0.8 floor the deploy gate + the M9 escalation use). Off by default, injectable client, a
no-op offline — so building the baseline and the smoke tests stay AWS-free.
"""
...
# =============================================================================
# Module 14 — emit the run's grounding as the prod-canary metric (by addition).
# =============================================================================
def emit_grounding_metric(result: dict[str, Any], *, cloudwatch=None) -> dict[str, Any]:
    """Publish the run's aggregate grounding as the EvalGrounding CloudWatch metric (M14).

    The golden set is also a PRODUCTION tool: re-run on the prod environment, its aggregate
    grounding is the quality canary the `relay-ops` grounding<0.8 alarm watches (the SAME 0.8
    constant as the deploy gate + the M9 escalation). This wraps observability.metrics.
    emit_eval_grounding — injectable client, a documented no-op offline, so importing run_evals
    in a test never makes a silent AWS call. Returns the metric dict. No model ID here; the
    metric namespace/name live in relay.config.
    """
    from observability import metrics as metrics_mod

    if cloudwatch is None:
        cloudwatch = _live_cloudwatch()
    grounding = float(result["aggregate"]["grounding"])
    metric = metrics_mod.emit_eval_grounding(grounding, cloudwatch=cloudwatch)
    print(f"  emitted {metric['name']}={metric['value']} to CloudWatch "
          f"({config.RELAY_METRIC_NAMESPACE}) — the relay-ops grounding<"
          f"{config.ALARM_GROUNDING_THRESHOLD} alarm watches it.")
    return metric


def _live_cloudwatch():
    """A CloudWatch client from the boto3 default session (only built when emitting live)."""
    import boto3
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--emit-metrics", action="store_true",
                   help="MODULE 14: publish the run's aggregate grounding to CloudWatch as the "
                        "EvalGrounding metric (the prod-canary signal the relay-ops "
                        "grounding<0.8 alarm watches). Needs AWS creds; off by default so "
                        "offline runs spend nothing.")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    # MODULE 14 (by addition): publish this run's aggregate grounding as the EvalGrounding
    # metric — the golden set used as a PRODUCTION CANARY (skill 4.3.6). The relay-ops
    # grounding<0.8 alarm (the ONE M9/M13 constant) watches it. Off by default so an offline
    # run spends nothing; --emit-metrics + AWS creds publishes one data point.
    if args.emit_metrics:
        emit_grounding_metric(result)
