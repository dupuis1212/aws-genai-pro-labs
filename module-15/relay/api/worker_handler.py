...
MODULE 14 ADDITION — the worker also EMITS Relay's custom CloudWatch metrics (bible §2.2
M14), BY ADDITION, from facts it already has at the end of a ticket: the metered cost
($/ticket), the token usage (the CostMeter's per-call totals), whether the ticket escalated,
whether the guardrail intervened, and the cited source (for the grounding-by-citation Logs
Insights query). It uses EMF (observability.metrics.emit_emf) — a structured log line the
CloudWatch agent turns into metrics for FREE, so it adds NO extra API call and NO latency on
the customer's ticket. The dashboard (`relay-ops`) and the alarms read these metrics. This
is purely additive: no model ID, no new generation call, no schema change — observability
emits what the worker computed, it does not re-derive it. Metric emission is BEST-EFFORT
(an observability failure never fails a shipped ticket).
"""
...
def emit_ticket_metrics(response: dict, meter: CostMeter, *, printer=print) -> dict:
    """Emit Relay's custom CloudWatch metrics for ONE ticket via EMF (Module 14). BEST-EFFORT.

    Reads facts the worker already has — the metered cost + per-call token totals (the
    CostMeter), the final status / escalation / guardrail intervention / cited source (the
    run_relay record) — and logs ONE EMF line (observability.metrics.emit_emf). The CloudWatch
    agent turns that line into the $/ticket, token, escalation-rate, guardrail-block-rate, and
    tool-latency metrics the `relay-ops` dashboard + alarms read — no extra API call, no
    latency on the ticket. Purely additive: no model ID, no new generation call, no schema
    change. Returns the emitted EMF dict (or {} on a no-op / failure). An observability hiccup
    NEVER fails a shipped ticket — the answer already went out; metrics are operational
    bookkeeping. `printer` is injectable so tests capture the line offline.
    """
    try:
        from observability import metrics as metrics_mod

        record = response.get("record") or {}
        # Token totals are the SUM over every converse() call the meter recorded (triage +
        # answer + agent loops) — the FM signal classic infra omits.
        input_tokens = sum(int(c.get("input_tokens", 0)) for c in meter.calls)
        output_tokens = sum(int(c.get("output_tokens", 0)) for c in meter.calls)
        escalated = bool(record.get("escalated")) or response.get("status") == "escalated"
        # The guardrail intervened on this ticket if run_relay surfaced it in the response
        # (the M9 guardrail_action, when present) — surfaced here as the dashboard's block-rate
        # signal. The frozen TicketRecord has no guardrail field, so we read the response's
        # optional `guardrail_action` (None/"NONE" -> not blocked) rather than invent a field.
        guardrail_blocked = response.get("guardrail_action") not in (None, "NONE")
        # The cited source (first citation, if any) — a CONTEXT field for the grounding-by-
        # citation Logs Insights query (NOT a metric dimension, so it costs nothing).
        answer = record.get("answer") or {}
        citations = answer.get("citations") or []
        cited_source = citations[0].get("source_uri") if citations else None

        ticket_metrics = metrics_mod.ticket_metrics(
        ...
        extra = {"ticket_id": response.get("ticket_id"),
                 "status": response.get("status")}
        if cited_source:
            extra["cited_source"] = cited_source
        return metrics_mod.emit_emf(ticket_metrics, extra=extra, printer=printer)
    except Exception as err:  # noqa: BLE001 — observability never fails a shipped ticket.
        print(f"[worker] metric emit skipped: {type(err).__name__}: {err}")
        return {}


def process_record(record: dict, *, run=None, events_client=None, bus_name=None,
                   cost_table=None, metrics_printer=print) -> dict:
    ...
    ticket's real cost; that cost is then written onto the persisted record (Module 12) AND
    emitted as CloudWatch metrics (Module 14, by addition). The returned dict carries the
    run_relay response plus `event` (the published detail-type or None) and `cost_cents` (the
    metered cost). Raises on a broken job / agent failure so SQS redelivers (then DLQs) — the
    worker never silently drops. `cost_table` / `metrics_printer` are injectable.
    """
    if run is None:
    ...
    # MODULE 14 — emit the ticket's custom metrics (EMF). Best-effort, after the cost is
    # written; an observability failure never fails the ticket.
    emit_ticket_metrics(response, meter, printer=metrics_printer)
