...
def write_received(ticket: Ticket, *, persist=None) -> dict:
    ...
    MODULE 15 HARDENING (by addition): the front door uses the IDEMPOTENT first-write
    (store.create_ticket_first_seen — a conditional PutItem on ticket_id) so a CloudCart
    webhook redelivery of the SAME ticket is rejected as a replay instead of starting a
    second pipeline (and a second refund). The `received` write is the right place for the
    idempotency gate: it is the FIRST write of a ticket's life. The agent's later status
    writes keep using the plain upsert to ADVANCE the same row. Callers/tests may inject
    `persist` to override (the M11 tests still pass the plain create_ticket).
    """
    if persist is None:
        from mcp_server import store

        persist = store.create_ticket_first_seen
    return persist(
        ticket.ticket_id,
        status="received",
...
def handle(event: dict, *, sqs_client=None, persist=None, queue_url=None) -> dict:
    ...
    # MODULE 15 HARDENING — idempotency: a duplicate webhook delivery (same ticket_id) is a
    # REPLAY, not a new ticket. The conditional first-write (write_received) raises
    # IdempotentReplay; we treat it as a no-op success (return 202 with the SAME ticket_id,
    # do NOT enqueue a second job) so the duplicate cannot start a second pipeline or a second
    # refund. The original ticket is already queued/processing.
    from mcp_server.store import IdempotentReplay
