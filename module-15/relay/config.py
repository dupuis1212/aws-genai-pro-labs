...
# =============================================================================
# Module 14 ADDITIONS — observability: model invocation logging, the custom-metric
# namespace + metric names, the `relay-ops` dashboard, the four alarms, and the
# injected-fault fixtures' marker.
# =============================================================================
# Added BY ADDITION in Module 14 (operating GenAI in production — observability,
# monitoring, troubleshooting). NOTHING above this line changes: the tier ->
# inference-profile map is FROZEN at Module 3 (Module 14 OBSERVES the same calls — it
# adds no tier and re-points nothing), the resource names + embedder at M4, the
# KB/reranker at M5, the intake policy at M6, the tables + MCP URL at M7, the AgentCore
# names at M8, the guardrail + the ONE grounding threshold (0.8) at M9, the PII/IAM at
# M10, the SQS queue + relay-events bus + pipeline at M11, the cache table + Flex tier
# at M12, the eval role + RAG-eval job + gate constants at M13. No model ID is named
# here (the grep gate covers config.py like every other file): observability watches
# Relay's existing calls; it never makes a new generation call of its own.
#
# This block appends only: the CloudWatch custom-metric NAMESPACE + the metric NAMES the
# worker / run_evals emit, the dashboard NAME (`relay-ops`, 06 §2 / bible §3.3), the four
# alarm NAMES + their thresholds (the grounding alarm REUSES the one M9/M13 0.8 constant —
# gate <-> alarm <-> escalation coherence), the SNS topic name alarms notify, the model-
# invocation-logging log group + delivery-status log group names, and the injected-fault
# state marker. Same containment law: ONE place each value lives, so observability/
# setup_observability.py, observability/inject_fault.py, relay/api/worker_handler.py,
# evals/run_evals.py, setup.py, and teardown.py all agree by construction.
#
# Boundary (brief §6 / §9): Model Invocation Logs (Bedrock request/response payloads, to
# CloudWatch Logs) are NOT CloudTrail (management-API audit, M10) — distinct sources, both
# named here so the distinction is enforced, not just described. Invocation logging is
# FREE on the Bedrock side (you pay only CloudWatch Logs storage — cents); never invent an
# activation price. CloudWatch generative AI observability (GA re:Invent 2025) is the
# native LLM/agent tracing path — this config holds the names; no homemade pre-GA metric-
# filter parsing of prompt text lives anywhere (brief §9).

# --- The custom-metric namespace + the metric names Relay emits ----------------
# Relay publishes its BUSINESS + FM metrics (the ones classic infra dashboards miss) under
# ONE CloudWatch namespace, so the `relay-ops` dashboard and the alarms reference them by a
# single name. EMF (Embedded Metric Format) / PutMetricData both land here. ONE place so the
# emitter (relay/api/worker_handler.py, evals/run_evals.py) and the dashboard/alarm builders
# (observability/setup_observability.py) never drift on the namespace or a metric name.
RELAY_METRIC_NAMESPACE = "Relay/Ops"

# The metric NAMES. Each answers a QUESTION the dashboard widget poses (brief §6 step 4),
# not decoration. CloudWatch bills per custom metric (cardinality matters — brief §7), so
# this is a DELIBERATELY SMALL, named set, not a metric per ticket field.
#   - CostCents       : $/ticket — the M12 cost_cents the worker already meters, now ALSO
#                       emitted as a metric (the dashboard's business line).
#   - InputTokens /   : token usage in/out per ticket — the FM signal classic infra omits;
#     OutputTokens      a sudden burst is the cost-anomaly canary (skill 4.3.2).
#   - Escalated       : 1 when a ticket escalated, 0 otherwise -> the escalation RATE (a
#                       business KPI: more escalations = Relay deflecting fewer tickets).
#   - GuardrailBlocked: 1 when the guardrail intervened on this ticket, else 0 -> the
#                       guardrail block RATE (M9 safety signal on the dashboard).
#   - EvalGrounding   : the last eval run's AGGREGATE grounding (run_evals emits it) — the
#                       PROD-canary quality signal the grounding<0.8 alarm watches.
#   - ToolLatencyMs   : the agent's tool-call latency (skill 4.3.4 — tool-calling
#                       observability the classic ML dashboard has no concept of).
METRIC_COST_CENTS = "CostCents"
METRIC_INPUT_TOKENS = "InputTokens"
METRIC_OUTPUT_TOKENS = "OutputTokens"
METRIC_ESCALATED = "Escalated"
METRIC_GUARDRAIL_BLOCKED = "GuardrailBlocked"
METRIC_EVAL_GROUNDING = "EvalGrounding"
METRIC_TOOL_LATENCY_MS = "ToolLatencyMs"

# The dimension every Relay metric carries: which Relay component emitted it. A small,
# bounded dimension set (Service=Relay) keeps metric CARDINALITY — and therefore the
# CloudWatch bill (brief §7) — low; we deliberately do NOT add ticket_id as a dimension
# (that would mint a metric per ticket). ONE place so the emitter and the dashboard agree.
METRIC_DIMENSION_SERVICE = "Service"
METRIC_SERVICE_VALUE = "Relay"

# --- The `relay-ops` dashboard (CloudWatch — 06 §2 / bible §3.3, 8 widgets) ------
# The frozen dashboard NAME. setup_observability.py builds it (8 widgets — tokens in/out by
# tier, $/ticket, p95 API latency, errors/throttling, escalation rate, guardrail block rate,
# eval grounding, agent tool latency); teardown.py deletes it (DeleteDashboards is free, but
# the course rule is leave nothing behind, B5). Canonical name — the article, the runbook,
# and the alarm-action descriptions all reference exactly `relay-ops`.
RELAY_DASHBOARD_NAME = "relay-ops"

# The number of widgets the dashboard MUST carry (brief §6 step 4 / bible §3.3). The smoke
# test asserts the built definition has exactly this many widgets, so a dropped widget is a
# test failure, not a silent regression.
RELAY_DASHBOARD_WIDGET_COUNT = 8

# --- The four alarms (brief §6 step 5) -----------------------------------------
# Alarm NAMES (one place, so the builder and teardown agree) + their thresholds. Each alarm
# links to a runbook entry (docs/runbook.md) — "an alarm without a runbook entry is just
# scheduled panic" (the article's assumed opinion). The four:
#   1. p95 API latency > threshold  (the "40-second answer" symptom)
#   2. throttling > 0 over 5 min    (ThrottlingException bursts — skill 5.2.2)
#   3. cost anomaly                 (ANOMALY DETECTION on daily cost — token-burst / cost
#                                    spike, skill 4.3.2; a band, not a static line)
#   4. eval grounding < 0.8         (the PROD-canary quality alarm — REUSES the ONE M9/M13
#                                    GROUNDING_THRESHOLD; gate <-> alarm <-> escalation
#                                    coherence, bible §4 M9)
ALARM_P95_LATENCY = "relay-ops-p95-latency"
ALARM_THROTTLING = "relay-ops-throttling"
ALARM_COST_ANOMALY = "relay-ops-cost-anomaly"
ALARM_GROUNDING = "relay-ops-grounding"

# The four alarm names as a tuple, so setup creates and teardown deletes the SAME set by
# iterating one list (no drift — the M10 IAM-roles pattern).
RELAY_ALARM_NAMES: tuple[str, ...] = (
    ALARM_P95_LATENCY,
    ALARM_THROTTLING,
    ALARM_COST_ANOMALY,
    ALARM_GROUNDING,
...
# p95 API latency alarm threshold (milliseconds). A support answer that takes longer than
# this is the article's "40 s to respond" symptom — worth waking someone. 10 s is generous
# for an async agent loop (the customer polls GET); tune in "Try it yourself". ONE place.
ALARM_P95_LATENCY_THRESHOLD_MS = 10_000

# The throttling alarm fires when ThrottlingException count > 0 within the evaluation period
# (a single throttle in steady state is a quota/backoff signal — skill 5.2.2). The period is
# 5 minutes (300 s), matching the brief's "throttling > 0 on 5 min".
ALARM_THROTTLING_THRESHOLD = 0
ALARM_PERIOD_SECONDS = 300

# The grounding alarm threshold IS the one M9/M13 0.8 constant (defined once at M9 as
# GROUNDING_THRESHOLD, reused as the M13 EVAL_GROUNDING_FLOOR, reused HERE as the M14
# `relay-ops` alarm). NEVER a divergent literal — gate <-> alarm <-> escalation coherence
# (bible §4 M9). A run whose aggregate grounding drops below this trips the alarm exactly as
# the same number trips the M13 deploy gate and the M9 per-answer escalation.
ALARM_GROUNDING_THRESHOLD = GROUNDING_THRESHOLD     # = 0.8 (the ONE M9 constant)

# The cost-anomaly alarm's anomaly-detection band width (standard deviations). CloudWatch
# anomaly detection learns the metric's normal range and alarms when a value falls outside a
# band this many std-devs wide — the "cost anomaly detection" the brief asks for (skill
# 4.3.2), NOT a static dollar line a growing app would trip on every busy day. 2 std-devs is
# the CloudWatch default; the lab keeps it. ONE place so the alarm and the article agree.
ALARM_COST_ANOMALY_BAND_STDDEV = 2

# --- The SNS topic alarms notify (email) ---------------------------------------
# The alarms publish to ONE SNS topic; a subscribed email is the on-call signal (the brief's
# "alarme -> email SNS suffit" — no PagerDuty). setup_observability.py creates the topic and,
# if RELAY_BUDGET_EMAIL is set, subscribes the address (the SAME env var the M1 budget alarm
# uses). teardown.py deletes the topic. ONE place for the name.
RELAY_ALARM_TOPIC_NAME = "relay-ops-alarms"

# The env var an operator sets to receive alarm emails (reused from M1's budget alarm — one
# email convention across the course). Optional: with no address, the topic is still created
# (alarms still fire and show on the dashboard); only the email subscription is skipped.
RELAY_ALARM_EMAIL_ENV = "RELAY_BUDGET_EMAIL"

# --- Model invocation logging (Bedrock -> CloudWatch Logs — 06 §4 / brief §6 step 2) --
# Bedrock's model-invocation logging writes every Converse request + response (and any other
# model-invocation API) to a CloudWatch Logs LOG GROUP (and/or S3). It is FREE on the Bedrock
# side — you pay only the
# CloudWatch Logs storage (cents). setup_observability.py calls
# put_model_invocation_logging_configuration pointing at this log group; teardown.py disables
# logging (delete_model_invocation_logging_configuration) and deletes the log group. The
# delivery-status SUB-group is where Bedrock reports log-delivery problems (a required
# companion group). ONE place so setup, the Logs Insights queries, and teardown agree.
#
# This is NOT CloudTrail (M10 management-event audit) — the exam's favourite distinction
# (brief §7 traps). Model Invocation Logs carry the PROMPT + RESPONSE (skill 4.3.2); they
# are sensitive (link M10 PII) and so get a SHORT retention (below). CloudTrail logs the
# bedrock:Converse API CALL, not its content.
RELAY_INVOCATION_LOG_GROUP = "/relay/bedrock/model-invocations"
RELAY_INVOCATION_DELIVERY_LOG_GROUP = "/relay/bedrock/model-invocations-delivery"

# How long the invocation logs are retained (days). Prompts/responses are sensitive AND
# voluminous (brief §7), so retention is SHORT — long enough to troubleshoot a recent
# incident with Logs Insights, short enough not to hoard sensitive payloads. 14 days. ONE
# place so setup sets it on the log group and the article's retention note matches.
RELAY_INVOCATION_LOG_RETENTION_DAYS = 14

# The IAM service role Bedrock assumes to deliver invocation logs to CloudWatch Logs
# (Bedrock logging service-role pattern). Created by setup_observability.py from a least-
# privilege policy, deleted by teardown.py — same one-place-per-name law as the M10 component
# roles, the M12 batch role, and the M13 eval role.
RELAY_INVOCATION_LOG_ROLE_NAME = "relay-bedrock-logging-role"

# --- The injected-fault state marker (brief §6 step 7) -------------------------
# inject_fault.py records which fault (if any) is currently injected to a git-ignored marker,
# so `--restore` knows what to undo and the runbook reader can see the current state. NEVER a
# literal fault baked into code: a fault is a visible, reversible FIXTURE (brief §9), and the
# marker is the visible record of it. teardown.py removes the marker (and restores any active
# fault first). ONE place for the name.
RELAY_FAULT_STATE_FILE_NAME = ".injected_fault"

# The three injected faults (brief §6 step 7), each mapping to a runbook entry. NAMES are the
# `--fault <name>` CLI values; ONE place so inject_fault.py, the runbook, and the smoke test
# (which asserts --list returns exactly these three) agree.
FAULT_CONTEXT_OVERFLOW = "context-overflow"
FAULT_KB_CORRUPTION = "kb-corruption"
FAULT_PROMPT_REGRESSION = "prompt-regression"

RELAY_INJECTED_FAULTS: tuple[str, ...] = (
    FAULT_CONTEXT_OVERFLOW,
    FAULT_KB_CORRUPTION,
    FAULT_PROMPT_REGRESSION,
)
