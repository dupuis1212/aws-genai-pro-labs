...
# =============================================================================
# Module 14 ADDITION — Relay's observability layer (delegates to observability/).
# =============================================================================
# Additive on top of everything above. Module 14 (operating GenAI in production) adds the
# OPS layer: Bedrock model-invocation logging (-> CloudWatch Logs, FREE on the Bedrock side),
# the `relay-ops` dashboard (8 widgets), four alarms (p95 latency, throttling, cost anomaly,
# grounding<0.8 — the ONE M9/M13 constant), and an SNS email topic. setup.py DELEGATES to
# observability/setup_observability.py (the brief: "setup.py délègue à setup_observability.py")
# so the wiring lives in ONE place. No model ID, no new generation call — observability watches
# Relay's existing calls. teardown.py removes all of it (B5). For cross-service tracing on the
# API->SQS->agent path, X-Ray is the AWS tool (renvoi M11 boundary tracing); this module builds
# the FM-layer signals instead.
def module_14_setup(*, account: str) -> None:
    """Wire Relay's observability layer (invocation logging + dashboard + alarms). Verbose.

    Called by main() after the M13 eval setup. Delegates to observability/setup_observability.py
    (one home for the dashboard/alarm/logging wiring) and prints the dashboard URL. Idempotent.
    """
    from observability import setup_observability as obs

    print("\nModule 14 — observability (invocation logging + `relay-ops` dashboard + alarms):")
    url = obs.setup_observability(account=account)
    print(f"\n  dashboard '{config.RELAY_DASHBOARD_NAME}': {url}")
    print("  inject a fault and follow docs/runbook.md:")
...
def main(argv: list[str] | None = None) -> int:
    ...
    skip_observability = "--skip-observability" in argv
    ...
    if leftover:
        print(f"Unknown argument(s): {' '.join(leftover)}\n"
              ...
              "[--skip-eval] [--submit-eval] [--skip-observability]\n"
    ...
    try:
        ...
        # --- Module 14: invocation logging + the relay-ops dashboard + alarms --------
        if not skip_observability:
            module_14_setup(account=acct)
        else:
            print("\n--skip-observability: leaving the M14 dashboard/alarms/logging untouched.")
    except ClientError as err:
        code = err.response["Error"]["Code"]
    ...
    if not skip_observability:
        print(f"  dashboard      : {config.RELAY_DASHBOARD_NAME} (CloudWatch, "
              f"{config.RELAY_DASHBOARD_WIDGET_COUNT} widgets; deleted at teardown)")
        print(f"  alarms         : {', '.join(config.RELAY_ALARM_NAMES)} "
              f"(-> SNS '{config.RELAY_ALARM_TOPIC_NAME}'; deleted at teardown)")
        print(f"  invocation logs: {config.RELAY_INVOCATION_LOG_GROUP} (Bedrock -> CloudWatch "
              f"Logs, {config.RELAY_INVOCATION_LOG_RETENTION_DAYS}-day retention; FREE on "
              "Bedrock, Logs storage cents; disabled + purged at teardown)")
        print(f"  logging role   : {config.RELAY_INVOCATION_LOG_ROLE_NAME} (IAM, Bedrock "
              "logging service role; FREE)")
