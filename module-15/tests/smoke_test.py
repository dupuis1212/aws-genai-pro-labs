"""smoke_test.py — Module 15 lab tests (cumulative: Modules 2–15).
...
def test_no_M16_or_later_capabilities_in_lab_code():
    """Module 15 IS the capstone — it BUILDS the 20-ticket `demo_capstone` and the `v1.0`
    release, so `demo_capstone` is now EXPECTED (no longer forbidden). What M15 must NOT USE
    is a DOWNSTREAM (M16) capability: M16 is the exam module (mock exam / revision grid /
    debrief) and ships NO Relay code — there is no M16 lab token to leak into here. The
    always-banned agent tooling (legacy starter-toolkit, Bedrock Agents classic
    create_agent/invoke_agent, LangChain) and the banned legacy invoke path remain the
    forbidden TOKENS in the lab CODE.

    The M11 front door + the M12 levers + the M13 eval harness + the M14 ops layer + the M15
    capstone demo/hardening are ALLOWED — they are the built increments. Scope = the lab code
    (relay/, evals/, observability/, mcp_server/, cdk/, pipeline/, setup.py, teardown.py,
    audit_report.py, cost_report.py, demo_capstone.py).
    """
    # The always-banned legacy tooling. The legacy invoke path is still banned everywhere but
    # ingest/embed.py (Titan embeddings — the course's sole exception). demo_capstone (M15) is
    # now BUILT, so it is NO LONGER forbidden. M16 introduces no lab code, so there is no new
    # forward token to block here.
    pattern = re.compile(
    ...
    lab_files += [_ROOT / "setup.py", _ROOT / "teardown.py", _ROOT / "audit_report.py",
                  _ROOT / "cost_report.py", _ROOT / "demo_capstone.py"]
    offenders = []
    ...
    # is PRESENT; the M15 capstone demo is now BUILT (forward boundary moved to M16).
    ...
    assert (_ROOT / "demo_capstone.py").exists()            # M15 build (the capstone demo)
...
# ===========================================================================
# Module 15 — the CAPSTONE: assemble + harden + review + demonstrate + ship.
# Offline by default (moto + a scripted local runner); ONE capped live marker.
# The capstone CONSUMES every frozen contract; it adds idempotency/timeouts/quotas
# AROUND them and the 20-ticket costed demo — it mutates no signature or schema.
# ===========================================================================
import demo_capstone as demo_mod  # noqa: E402

DEMO_PATH = _ROOT / "demo_capstone.py"
LENS_REVIEW_PATH = _ROOT / "docs" / "genai-lens-review.md"


def test_capstone_demo_lists_exactly_twenty_tickets():
    """`demo_capstone.py --list` returns EXACTLY 20 tickets (brief §6 — the demo size is
    load-bearing for coverage). No AWS call; pure fixture loading."""
    tickets = demo_mod.build_demo_tickets()
    assert len(tickets) == 20, f"expected 20 demo tickets, got {len(tickets)}"
    # Every ticket has a non-empty message + a valid channel + a category.
    for t in tickets:
        assert t.customer_message.strip(), t.ticket_id
        assert t.channel in ("email", "chat"), t.ticket_id
        assert t.category in ("nominal", "adversarial", "multimodal"), t.ticket_id
    # The --list CLI path runs and reports 20.
    rc = demo_mod.main(["--list"])
    assert rc == 0


def test_capstone_demo_mixes_nominal_adversarial_and_multimodal():
    """The 20-ticket mix MUST exercise adversarial[M9] + multimodal[M6] tickets, not just the
    happy path (bible §5.7 — those skills count M15 as their 2nd touchpoint)."""
    tickets = demo_mod.build_demo_tickets()
    by_cat: dict[str, int] = {}
    for t in tickets:
        by_cat[t.category] = by_cat.get(t.category, 0) + 1
    assert by_cat.get("adversarial", 0) >= 2, by_cat   # reused from data/attacks.json (M9)
    assert by_cat.get("multimodal", 0) >= 2, by_cat    # reused from data/raw screenshots (M6)
    assert by_cat.get("nominal", 0) >= 12, by_cat
    # The adversarial tickets are the real M9 attacks; the multimodal ones carry a screenshot.
    adv = [t for t in tickets if t.category == "adversarial"]
    assert any("ignore your previous instructions" in t.customer_message.lower() for t in adv)
    multi = [t for t in tickets if t.category == "multimodal"]
    assert all(t.attachment and t.attachment.endswith(".png") for t in multi)


def test_capstone_hardening_idempotency_one_record_on_duplicate(dynamodb_backend):
    """HARDENING — idempotency (brief T5.2): two submissions of the SAME ticket produce ONE
    TicketRecord and the SECOND raises IdempotentReplay (a conditional DynamoDB write), never a
    duplicate row or a second action. This is the capstone's headline durability proof."""
    tid = "cap-idem-001"
    # First delivery: the conditional first-write succeeds.
    rec1 = store_mod.create_ticket_first_seen(
        tid, status="received", resource=dynamodb_backend)
    assert rec1["ticket_id"] == tid and rec1["status"] == "received"
    # Second delivery of the SAME ticket: rejected as an idempotent replay.
    with pytest.raises(store_mod.IdempotentReplay):
        store_mod.create_ticket_first_seen(
            tid, status="received", resource=dynamodb_backend)
    # Exactly ONE row for the ticket (the duplicate did not clobber or duplicate it).
    table = dynamodb_backend.Table(config.RELAY_TICKETS_TABLE)
    items = [i for i in table.scan().get("Items", []) if i["ticket_id"] == tid]
    assert len(items) == 1
    # The agent's later status advances still upsert the SAME row (received -> answered),
    # so the lifecycle is never blocked — only a NEW first-write is gated.
    store_mod.create_ticket(tid, status="answered", resource=dynamodb_backend)
    items = [i for i in table.scan().get("Items", []) if i["ticket_id"] == tid]
    assert len(items) == 1 and items[0]["status"] == "answered"


def test_post_handler_idempotent_on_duplicate_webhook(dynamodb_backend, sqs_backend):
    """The deployed front door is idempotent: a duplicate POST of the same ticket_id is a
    no-op (202 with duplicate=True), enqueuing NO second job — so a CloudCart webhook
    redelivery cannot start a second pipeline or a second refund (brief T5.2)."""
    sqs_client, queue_url = sqs_backend
    body = {"customer_message": "Please refund order 1042.", "ticket_id": "cap-dup-1"}
    persist = lambda tid, **kw: store_mod.create_ticket_first_seen(  # noqa: E731
        tid, resource=dynamodb_backend, **kw)

    r1 = post_handler.handle(_proxy_event(body=body), sqs_client=sqs_client,
                             queue_url=queue_url, persist=persist)
    assert r1["statusCode"] == 202 and "duplicate" not in json.loads(r1["body"])

    # The DUPLICATE delivery: same id, idempotent no-op, no second enqueue.
    r2 = post_handler.handle(_proxy_event(body=body), sqs_client=sqs_client,
                             queue_url=queue_url, persist=persist)
    assert r2["statusCode"] == 202
    assert json.loads(r2["body"]).get("duplicate") is True

    # Exactly ONE job on the queue (the duplicate did not enqueue a second).
    msgs = sqs_client.receive_message(QueueUrl=queue_url, MaxNumberOfMessages=10)
    assert len(msgs.get("Messages", [])) == 1
    # And exactly ONE relay-tickets row.
    table = dynamodb_backend.Table(config.RELAY_TICKETS_TABLE)
    rows = [i for i in table.scan().get("Items", []) if i["ticket_id"] == "cap-dup-1"]
    assert len(rows) == 1


def test_capstone_demo_recap_math_offline_with_a_scripted_runner():
    """The demo orchestration + recap math run OFFLINE with a scripted run_relay: status
    counts, escalation rate, total $/ticket, and p95 are computed correctly. No AWS, no
    Bedrock — the scripted runner returns a deterministic frozen response per ticket."""
    def scripted_run(payload):
        # A deterministic run_relay-shaped response: answered, no cost (offline), one citation.
        tid = payload["ticket_id"]
        status = "escalated" if "attack" in tid else "answered"
        return {
            "ticket_id": tid, "status": status, "answer_text": "ok",
            "handed_off": False, "gated": False,
            "record": {"ticket_id": tid, "status": status, "escalated": status == "escalated",
                       "triage": {"intent": "billing", "priority": "normal",
                                  "sentiment": "neutral"},
                       "answer": {"text": "ok", "citations": [
                           {"source_uri": "s3://x", "snippet": "y"}], "grounded": True},
                       "actions": [], "cost_cents": 0.0},
        }
    # Run all 20 through the scripted runner; skip the live eval score.
    outcomes = [demo_mod.run_ticket_local(t, run=scripted_run)
                for t in demo_mod.build_demo_tickets()]
    assert len(outcomes) == 20
    recap = demo_mod.summarise(outcomes, eval_score=0.95)
    assert recap["tickets"] == 20
    # The two adversarial tickets escalated; escalation rate = 2/20 = 0.1.
    assert recap["by_status"].get("escalated", 0) == 2
    assert recap["escalation_rate"] == 0.1
    assert recap["eval_grounding"] == 0.95
    assert recap["p95_latency_ms"] >= 0
    assert recap["total_cost_usd"] >= 0


def test_genai_lens_review_exists_and_covers_every_pillar():
    """docs/genai-lens-review.md is the graded M15 artifact (skill 1.1.3): a checklist that
    covers ALL SIX Well-Architected pillars + the GenAI-specific considerations, each with a
    status and a Relay control. Not prose — a reviewer can tick it."""
    assert LENS_REVIEW_PATH.exists(), "docs/genai-lens-review.md is missing (the M15 artifact)."
    text = LENS_REVIEW_PATH.read_text(encoding="utf-8")
    # The six official pillars (Well-Architected Generative AI Lens).
    for pillar in ("Operational Excellence", "Security", "Reliability",
                   "Performance Efficiency", "Cost Optimization", "Sustainability"):
        assert pillar in text, f"Lens review missing pillar: {pillar}"
    # The bold exam term at first use + the GenAI-specific considerations.
    assert "Well-Architected Generative AI Lens" in text
    assert "RAG grounding" in text or "RAG" in text
    assert "Human-in-the-loop" in text or "human-in-the-loop" in text.lower()
    # It records both satisfied controls (✅) and remaining gaps (⚠️ -> T7), not all-green.
    assert "✅" in text and "⚠️" in text
    # The idempotency / timeout / teardown hardening is recorded as a control.
    assert "Idempotency" in text or "idempoten" in text.lower()
    assert "teardown" in text.lower()


def test_capstone_v1_0_release_documented_in_readme():
    """The capstone's contract deliverable is the `v1.0` tag (06 §3). The README must document
    it: the pitch + the architecture + the run numbers + the teardown command."""
    readme = (_ROOT / "README.md").read_text(encoding="utf-8")
    assert "v1.0" in readme
    # The README points at the teardown (the ~$0/month proof) and the demo.
    assert "teardown" in readme.lower()
    assert "demo_capstone" in readme


def test_lab_md_states_the_measured_cost():
    """lab.md carries the MEASURED cost header (brief §6 / checklist): 'This lab cost me $X.XX
    on June 2026 prices'."""
    lab = (_ROOT / "lab.md").read_text(encoding="utf-8")
    assert "This lab cost me $" in lab
    assert "June 2026" in lab
    # The exhaustive teardown (incl. the AgentCore long-term Memory purge) is documented.
    assert "Memory" in lab and "teardown" in lab.lower()


def test_converse_signature_is_still_byte_identical_at_m15():
    """The capstone consumes converse() — its signature is byte-identical M3->M15 (the LAW).
    No hardening touched it."""
    import inspect

    sig = str(inspect.signature(llm.converse))
    assert sig == "(messages, *, tier='auto', stream=False, **params)", sig


# ===========================================================================
# Module 15 LIVE — ONE capped real end-to-end ticket through the assembled system.
# ===========================================================================
@pytest.mark.live
@pytest.mark.skipif(
    os.environ.get("RELAY_LIVE_TESTS") != "1",
    reason="set RELAY_LIVE_TESTS=1 to run ONE real capstone ticket end-to-end (< $0.02).",
)
def test_live_capstone_one_ticket_end_to_end():
    """ONE real ticket through the assembled system via the FROZEN run_relay seam (the exact
    path the deployed worker takes), metered. Call budget: one smart-tier agent loop (triage +
    answer, a few converse() calls, < $0.02 as of June 2026). Skips cleanly without creds.
    Proves the capstone's headline: a real ticket goes in and a costed, terminal-status
    TicketRecord comes out."""
    try:
        ticket = demo_mod.DemoTicket(
            "cap-live-001",
            "Where is my order 1042? It was supposed to arrive three days ago.",
            "nominal")
        outcome = demo_mod.run_ticket_local(ticket)
    except Exception as err:  # noqa: BLE001 — no creds / no network -> skip, not fail.
        pytest.skip(f"Bedrock not reachable ({type(err).__name__}); set AWS_PROFILE.")
    assert outcome.status in demo_mod.TERMINAL and outcome.status != "failed", outcome.error
