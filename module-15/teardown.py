...
# Module 13: the marker setup.py wrote the submitted RAG-eval job ARN to (git-ignored,
# account/run-specific). teardown stops the job (if any) and removes the marker.
EVAL_JOB_ARN_FILE = Path(__file__).resolve().parent / config.RELAY_EVAL_JOB_ARN_FILE_NAME
...
# =============================================================================
# Module 13 teardown — stop the RAG-eval job, drop the eval role + S3 artifacts.
# =============================================================================
# Module 13 added NO standing idle-billed resource — only the eval IAM service role (FREE)
# and small S3 artifacts (the RAG-eval dataset + report) under the data bucket's evals/
# prefix. B5 says leave nothing behind: STOP a still-running eval job (so it cannot keep
# spending tokens), delete the role, purge the artifacts. Always run (not gated) — the eval
# role + artifacts are M13's own, never reused upstream.
def stop_eval_job(bedrock_control) -> None:
    """Stop the recorded Bedrock RAG-evaluation job (so it stops consuming tokens). Idempotent.

    Reads the job ARN from the .eval_job_arn marker setup.py wrote; if the job is still
    IN_PROGRESS, StopEvaluationJob cancels it (a completed/missing/already-stopped job is a
    clean no-op). The marker is removed afterwards. A Bedrock eval job has no idle cost, but a
    LONG-running one keeps spending tokens — stopping it is the leave-nothing-behind rule."""
    if not EVAL_JOB_ARN_FILE.exists():
        print("  RAG-eval job: no .eval_job_arn marker — nothing to stop. Fine.")
        return
    arn = EVAL_JOB_ARN_FILE.read_text(encoding="utf-8").strip()
    try:
        bedrock_control.stop_evaluation_job(jobIdentifier=arn)
        print(f"  RAG-eval job {arn}: STOP requested.")
    except ClientError as err:
        code = err.response["Error"]["Code"]
        # ValidationException / ConflictException = already terminal; ResourceNotFound = gone.
        if code in _NOT_FOUND or code in ("ValidationException", "ConflictException"):
            print(f"  RAG-eval job {arn}: already terminal/gone ({code}). Fine.")
        else:
            raise
    except AttributeError:
        # Older botocore without stop_evaluation_job — the job is short; report and move on.
        print(f"  RAG-eval job {arn}: stop API unavailable in this botocore; "
              "it finishes on its own (no idle cost).")
    EVAL_JOB_ARN_FILE.unlink(missing_ok=True)
    print(f"  removed {EVAL_JOB_ARN_FILE.name}.")


def delete_eval_role(iam) -> None:
    """Delete the M13 Bedrock-evaluation service role (+ its inline policy). Idempotent.
    ...
    try:
        for name in iam.list_role_policies(RoleName=role_name).get("PolicyNames", []):
            ...
            print(f"  inline policy '{name}' on '{role_name}': deleted.")
        iam.delete_role(RoleName=role_name)
        print(f"  IAM role '{role_name}': DELETED (M13 eval role).")
    except ClientError as err:
        if err.response["Error"]["Code"] in _NOT_FOUND:
            print(f"  IAM role '{role_name}': already gone. Fine.")
        else:
...
def purge_eval_artifacts(s3, data_bucket: str) -> int:
    """Delete the RAG-eval job's S3 objects under evals/ (dataset + report). Idempotent.

    Returns the count removed. The eval dataset (evals/input/) and the job's report
    (evals/output/) are the ONLY idle-billed thing M13 adds (small S3 objects); this purges
    both so nothing lingers (B5). A missing prefix is a clean no-op. NOTE: this purges the S3
    `evals/` prefix in the DATA BUCKET — it does NOT touch the local repo's evals/ package or
    the committed evals/results/run-baseline.json (those are source, not infra)."""
    removed = 0
    for prefix in (config.RELAY_EVAL_INPUT_PREFIX, config.RELAY_EVAL_OUTPUT_PREFIX):
        try:
            paginator = s3.get_paginator("list_objects_v2")
            for page in paginator.paginate(Bucket=data_bucket, Prefix=prefix):
                keys = [{"Key": obj["Key"]} for obj in page.get("Contents", [])]
            ...
            raise
    if removed:
        print(f"  eval artifacts under evals/: {removed} object(s) DELETED.")
    else:
        print("  eval artifacts under evals/: none to remove. Fine.")
    return removed


# =============================================================================
# Module 14 teardown — delete the dashboard + alarms, disable invocation logging,
# delete the log groups + SNS topic + logging role, restore any injected fault.
# =============================================================================
# Module 14 added the OPS layer. Most of it is ~$0 idle (a dashboard, alarms, an SNS topic),
# but the course rule is leave nothing behind (B5), and the model-invocation logging keeps
# WRITING to CloudWatch Logs (storage cents) until disabled — so it MUST be turned off and the
# log groups purged. Always run (not gated) — these are M14's OWN resources, nothing upstream
# reuses them. Idempotent throughout. Order: restore any live fault (so the repo is clean),
# then dashboard -> alarms -> disable logging -> log groups -> SNS topic -> logging role.
def _cloudwatch():
...
def restore_injected_fault() -> None:
    """If an inject_fault.py fault is live, RESTORE it so teardown leaves a clean repo.

    A fault is a local, reversible fixture; leaving one injected would leave Relay broken after
    a teardown. We call the same restore() the CLI uses (it backs up/restores docs + clears the
    marker). A clean state is a no-op. Best-effort: a restore hiccup never blocks the AWS
    teardown."""
    try:
        from observability import inject_fault

        if inject_fault.current_fault() is not None:
            inject_fault.restore()
        else:
            print("  injected faults: none live. Fine.")
    except Exception as err:  # noqa: BLE001 — fault cleanup never blocks the AWS teardown.
        print(f"  injected-fault restore skipped: {type(err).__name__}: {err}")


def delete_dashboard(cloudwatch) -> None:
    """Delete the `relay-ops` CloudWatch dashboard. Idempotent (DeleteDashboards is free)."""
    name = config.RELAY_DASHBOARD_NAME
    try:
        cloudwatch.delete_dashboards(DashboardNames=[name])
        print(f"  dashboard '{name}': DELETED.")
    except ClientError as err:
        if err.response["Error"]["Code"] in _NOT_FOUND + ("ResourceNotFound",):
            print(f"  dashboard '{name}': already gone. Fine.")
        else:
            raise


def delete_alarms(cloudwatch) -> None:
    """Delete the four `relay-ops` alarms (by the config name list). Idempotent.

    DeleteAlarms ignores names that do not exist, so this is a clean no-op on a second run."""
    names = list(config.RELAY_ALARM_NAMES)
    cloudwatch.delete_alarms(AlarmNames=names)
    print(f"  alarms {', '.join(names)}: DELETED (missing ones ignored).")


def disable_invocation_logging(bedrock) -> None:
    """Turn OFF Bedrock model-invocation logging (so it stops writing to Logs). Idempotent.

    delete_model_invocation_logging_configuration removes the config; a missing config is a
    clean no-op. This is the cost-stopping step — left on, logging keeps billing Logs storage."""
    try:
        bedrock.delete_model_invocation_logging_configuration()
        print("  model invocation logging: DISABLED (stops writing to CloudWatch Logs).")
    except ClientError as err:
        if err.response["Error"]["Code"] in _NOT_FOUND + ("ValidationException",):
            print("  model invocation logging: already disabled. Fine.")
        else:
            raise


def delete_invocation_log_groups(logs) -> None:
    """Delete the two invocation log groups (the logs + the delivery-status group). Idempotent."""
    for name in (config.RELAY_INVOCATION_LOG_GROUP,
                 config.RELAY_INVOCATION_DELIVERY_LOG_GROUP):
        try:
            logs.delete_log_group(logGroupName=name)
            print(f"  log group '{name}': DELETED.")
        except ClientError as err:
            if err.response["Error"]["Code"] in _NOT_FOUND:
                print(f"  log group '{name}': already gone. Fine.")
            else:
                raise


def delete_alarm_topic(sns, account: str) -> None:
    """Delete the relay-ops-alarms SNS topic (+ its subscriptions). Idempotent.

    The topic ARN is account-specific; we build it from the frozen name + the account (the SNS
    ARN shape). A missing topic is a clean no-op."""
    arn = f"arn:aws:sns:{REGION}:{account}:{config.RELAY_ALARM_TOPIC_NAME}"
    try:
        sns.delete_topic(TopicArn=arn)
        print(f"  SNS topic '{config.RELAY_ALARM_TOPIC_NAME}': DELETED.")
    except ClientError as err:
        if err.response["Error"]["Code"] in _NOT_FOUND + ("NotFound",):
            print(f"  SNS topic '{config.RELAY_ALARM_TOPIC_NAME}': already gone. Fine.")
        else:
            raise


def delete_invocation_logging_role(iam) -> None:
    """Delete the M14 Bedrock-logging service role (+ its inline policy). Idempotent."""
    ...
    try:
        for name in iam.list_role_policies(RoleName=role_name).get("PolicyNames", []):
            ...
            print(f"  inline policy '{name}' on '{role_name}': deleted.")
        iam.delete_role(RoleName=role_name)
        print(f"  IAM role '{role_name}': DELETED (M14 logging role).")
    except ClientError as err:
        if err.response["Error"]["Code"] in _NOT_FOUND:
            print(f"  IAM role '{role_name}': already gone. Fine.")
        else:
...
def module_14_teardown(*, account: str, cloudwatch=None, logs=None, sns=None, iam=None,
                       bedrock=None) -> None:
    """Tear down the whole M14 ops layer. Clients injectable for offline tests. Idempotent."""
    ...
    restore_injected_fault()
    delete_dashboard(cloudwatch)
    delete_alarms(cloudwatch)
    disable_invocation_logging(bedrock)
    delete_invocation_log_groups(logs)
    delete_alarm_topic(sns, account)
    delete_invocation_logging_role(iam)
...
def assert_clean(*, drop_tables: bool, drop_kb: bool, drop_vectors: bool,
    ...
    print(f"  - Module 13 eval role '{config.RELAY_EVAL_ROLE_NAME}' (FREE) + the RAG-eval "
          f"job + evals/ S3 artifacts: DELETED above (M13's own resources; the committed "
          "evals/results baseline is source, never touched — B5).")
    print(f"  - Module 14 dashboard '{config.RELAY_DASHBOARD_NAME}' + {len(config.RELAY_ALARM_NAMES)} "
          f"alarms + SNS topic '{config.RELAY_ALARM_TOPIC_NAME}' + invocation logging "
          f"(log groups + role '{config.RELAY_INVOCATION_LOG_ROLE_NAME}'): DELETED/DISABLED "
          "above (M14's own resources; logging stopped writing to Logs — B5). Any injected "
          "fault was restored.")
    if drop_tables:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # --- Module 13: stop the RAG-eval job + drop the eval role + S3 artifacts -----
        # M13's OWN resources (the eval role + the evals/ S3 artifacts; nothing upstream
        # reuses them), so they go unconditionally (B5). The committed evals/results/
        # run-baseline.json + the evals/ package are SOURCE, not infra — never touched here.
        print("\nModule 13 — RAG-eval job + eval role + eval artifacts:")
        stop_eval_job(_bedrock_control())
        delete_eval_role(iam)
        purge_eval_artifacts(s3, data_bucket)

        # --- Module 14: dashboard + alarms + invocation logging + log groups + topic --
        # M14's OWN resources (the relay-ops dashboard, the four alarms, the SNS topic, the
        # invocation log groups + logging role). Most are ~$0 idle, but invocation logging
        # keeps WRITING to Logs until disabled, so it MUST go (B5). Also restores any
        # inject_fault.py fault so the repo is left clean. Unconditional (nothing upstream
        # reuses them).
        print("\nModule 14 — dashboard + alarms + invocation logging + injected faults:")
        module_14_teardown(account=acct)
