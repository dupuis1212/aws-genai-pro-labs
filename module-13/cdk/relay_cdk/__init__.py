...
M1-M10) to DECLARATIVE AWS CDK (decision B6). Two stacks:
  ...
  - pipeline_stack.RelayPipelineStack : the CodePipeline (source -> build -> deploy ->
                                   smoke -> EVAL-GATE) that re-deploys RelayApiStack on
                                   every commit. Module 13 WIRED the eval-gate stage (the
                                   golden-set regression gate) after smoke, before promotion.
