...
def test_no_M14_or_later_capabilities_in_lab_code():
    """Module 13 BUILDS the eval harness — the golden set, the LLM-as-a-judge, run_evals.py +
    the regression gate, and the user-feedback loop (feedback_rating + POST /feedback) are its
    OWN increment, so `evals/`, `run_evals`, `golden_set`, `feedback_rating`, and the wired
    eval-gate are now EXPECTED. What M13 must NOT USE is a DOWNSTREAM capability: no
    observability/dashboard/alarm wiring (M14 — observability/ does not exist yet), and no
    capstone demo (M15). Those, plus the forbidden agent tooling (legacy starter-toolkit,
    Bedrock Agents classic create_agent/invoke_agent, LangChain) and the banned legacy invoke
    path, are the forbidden TOKENS in the lab CODE.

    The M11 front door + the M12 levers + the M13 eval harness are ALLOWED — they are the
    built increments. Scope = the lab code (relay/, evals/, mcp_server/, cdk/, pipeline/,
    setup.py, teardown.py, audit_report.py, cost_report.py).
    """
    # The forbidden DOWNSTREAM (M14+) capabilities + the always-banned legacy tooling. The
    # legacy invoke path is still banned everywhere but ingest/embed.py (Titan embeddings —
    # the course's sole exception). NOTE: `relay-ops` is NOT in this regex — inherited
    # config/__init__ docstrings name the M14 dashboard as a forward-pointer (the 0.8
    # grounding constant is "reused by the M14 relay-ops alarm"), not as usage. The REAL M14
    # boundary (no observability/ package, no setup wired) is asserted structurally below.
    pattern = re.compile(
        r"starter.toolkit|create_agent|invoke_agent|invoke_model|"
        r"langgraph|langchain|"
        r"setup_observability|inject_fault|demo_capstone",
        re.IGNORECASE,
    )
    ...
    assert offenders == [], offenders
    # The M10 Ticket field still exists; the M13 feedback_rating is now PRESENT (built here);
    # the M14 observability package must STILL be absent (the forward-dependency boundary).
    assert "pii_redacted" in Ticket.model_fields
    assert "feedback_rating" in TicketRecord.model_fields  # M13 build
    assert not (_ROOT / "observability").exists()           # M14 boundary
...
# OpenAPI client integrates against. Module 13 ADDS the fourth route
# (POST /tickets/{ticket_id}/feedback, the user-feedback loop, skill 5.1.3), so the
# export now carries FOUR routes (06 §2 / bible §3.3).
...
def test_openapi_export_has_exactly_the_four_routes_with_feedback():
    """Module 13 ADDS POST /tickets/{ticket_id}/feedback, so the export carries EXACTLY
    four routes (the three M11 routes + /feedback). It matches the CDK WIRING SPEC
    field-for-field, and the FeedbackRequest schema is present."""
    spec = _load_openapi()
    routes = _openapi_routes(spec)

    assert routes == EXPECTED_OPENAPI_ROUTES, routes
    assert len(routes) == 4
    # /feedback IS in the M13 contract (the route this module builds).
