...
package the rest of the course builds on. By Module 3 it ships four pieces:
  ...
  - relay.cache  : Relay's SEMANTIC CACHE — a DynamoDB (on-demand) store + the pinned
                   Titan V2 embedder that answers a frequent, near-duplicate question
                   WITHOUT a fresh converse() call (second-time cost ~ 0). It distinguishes
                   the three caches the exam keeps separate: deterministic request hashing
                   (an exact-match key, zero risk), semantic caching (embedding similarity
                   >= a STRICT threshold — the real risk is a false hit, so never blind), and
                   it leaves PROMPT caching to relay.llm (a cached input prefix). A TTL +
                   invalidate() age entries out (passive + active invalidation). Added in
                   Module 12; holds NO model ID and makes NO generation call — embeddings go
                   through the Module 4 Titan path, the answer comes from the caller on a
                   miss. The semantic-cache table name + threshold + TTL live in relay.config.

  - relay.api    : Relay's serverless FRONT DOOR — four AWS Lambda handlers behind
                   Amazon API Gateway, wired async via Amazon SQS and loosely coupled via
                   Amazon EventBridge. post_handler (POST /tickets -> 202 {ticket_id},
                   enqueue), worker_handler (the SQS consumer -> invokes the deployed agent
                   through the FROZEN relay.run.run_relay contract -> final status ->
                   publishes relay.escalation / relay.approval_required to the relay-events
                   bus), get_handler (GET /tickets/{id} -> the TicketRecord), approve_handler
                   (POST /tickets/{id}/approve {approved: bool} -> the M8 HITL refund gate
                   over HTTP, calling relay.approve.approve). Added in Module 11. It WRAPS
                   the unchanged agent — it never re-implements it; holds NO model ID (all
                   generation stays in relay.llm via relay.run). The whole stack is described
                   in AWS CDK (cdk/) and deployed by a CodePipeline (pipeline/). No new
                   Pydantic field — Module 11 only EXERCISES the full frozen TicketRecord
...
relay.models, BY ADDITION. Module 11 adds the relay.api front-door package (four Lambda
handlers) that WRAPS the unchanged agent — async via SQS, loose coupling via EventBridge,
all in AWS CDK + a CodePipeline — adding the SQS-queue and relay-events bus NAMES to
relay.config BY ADDITION, with NO schema change. Module 12 adds relay.cache (the semantic
cache), adds prompt-caching + the Flex service tier to converse() via **params (signature
UNCHANGED), adds a per-ticket CostMeter to relay.llm, and finally POPULATES the frozen
TicketRecord.cost_cents (the M7 placeholder field) from the M3 per-tier price map in
relay.api.worker_handler — all BY ADDITION, with NO schema change (cost_cents already
exists). Module 13 adds the EVALUATION harness (the top-level evals/ package — golden set,
the LLM-as-a-judge, run_evals.py + the regression gate; NOT a relay/ submodule, so __all__
is unchanged), adds a FIFTH relay.api handler (POST /tickets/{id}/feedback) that wraps the
store, adds the judge model ID + the eval resource names to relay.config BY ADDITION (the
candidates stay the frozen fast/smart/vision tiers; the judge is a DIFFERENT model family —
self-preference bias is designed out), and adds EXACTLY ONE field —
`TicketRecord.feedback_rating: int | None = None` — to relay.models, BY ADDITION. Later
modules extend this package by ADDITION only. Nothing here is rewritten downstream.
"""

__all__ = [
    "models", "config", "llm", "triage", "kb", "intake", "tools", "agent",
    "specialists", "approve", "run", "safety", "pii", "cache", "api",
]
