"""smoke_test.py — Module 10 lab tests (cumulative: Modules 2, 3, 4, 5, 6, 7, 8, 9 and 10).
...
Together that is well under $0.08 as of June 2026. They need AWS credentials and
...
def test_no_M11_or_later_capabilities_in_lab_code():
    """M10 builds PII redaction (Comprehend DetectPiiEntities — relay/pii.py) and the
    least-privilege IAM roles — its OWN increment, so those tokens are now EXPECTED. But
    it must NOT USE a DOWNSTREAM capability: no public API / API Gateway / EventBridge
    `relay-events` bus (M11), no semantic cache (M12), no eval harness (M13). Those, plus
    the forbidden agent tooling (legacy starter-toolkit, Bedrock Agents classic
    create_agent/invoke_agent, LangChain) and the banned legacy invoke path, are
    forbidden TOKENS in the lab CODE. (brief §10 grep gate, shifted to the M10 boundary.)

    `DetectPiiEntities` / `pii_redacted` are ALLOWED at M10 (the increment itself); the
    forbidden list is exactly the legacy / M11+ out-of-scope tokens. Scope = the M10 lab
    code (relay/, mcp_server/, setup.py, teardown.py, audit_report.py) — inherited helper
    scripts carry their own benign prose (freshness_test.py mentions an EventBridge
    SCHEDULE for doc re-sync, unrelated to the M11 bus) and are byte-identical upstream.
    """
    pattern = re.compile(
        r"starter.toolkit|create_agent|invoke_agent|invoke_model|"
        r"langgraph|langchain|relay-events|api[_-]?gateway|eventbridge|"
        r"relay/cache|relay\.cache",
        re.IGNORECASE,
    )
    ...
    assert offenders == [], offenders
    # The M10 Ticket field NOW EXISTS (pii_redacted — this module's addition); the M13
    # feedback_rating must still be absent (the real forward-dependency boundary).
