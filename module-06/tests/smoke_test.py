"""smoke_test.py — Module 6 lab tests (cumulative: Modules 2, 3, 4, 5 and 6).
...
LIVE-CALL BUDGET: at most SIX calls total —
...
Together that is well under $0.02 (two cents) as of June 2026. They need AWS
credentials and us-east-1. NO live test creates or deletes a KB / ingestion job and
NONE uploads to S3 (the live vision call reads the screenshot from local bytes).
"""
...
# ===========================================================================
def test_module6_schema_set_is_m2_plus_citation_answer_plus_attachment():
    """M6 adds EXACTLY Attachment to the M2+M5 schemas — nothing more.

    AgentAction/TicketRecord are M7. Guarding the full set keeps the "by addition
    only, in order" invariant honest: at Module 6 the schemas are precisely
    ...
    assert schema_names == {"Ticket", "Triage", "Citation", "Answer", "Attachment"}
    # Ticket gained exactly `attachments`; still NO pii_redacted (M10).
    assert set(Ticket.model_fields) == {
        "ticket_id", "channel", "customer_message", "attachments", "created_at"
    }
...
def test_no_M7_or_later_capabilities_in_lab_code():
    """M6 may use Attachment / relay.intake / vision (its own increment), but must
    NOT USE a downstream CAPABILITY: the M7 tool token `search_kb`, the M7 schema
    classes (AgentAction, TicketRecord), or the M7 resource names
    (relay-orders / relay-tickets). Attachment is now LEGITIMATELY introduced here.

    `pii_redacted` (M10) is checked separately as a Ticket FIELD (it must not exist);
    the bare token still appears in M6 boundary comments ("Module 10's PII redaction
    slots in here") — a one-line forward-reference teaser, exactly like the inherited
    guardrail mentions in kb.py/llm.py — so it is NOT grepped here.
    """
    pattern = re.compile(
        r"\bsearch_kb\b|relay-orders|relay-tickets|"
        r"\bAgentAction\b|\bTicketRecord\b"
    )
    offenders = []
    for path in _ROOT.rglob("*.py"):
        if ".venv" in path.parts or path.name == "smoke_test.py":
        ...
        if pattern.search(path.read_text(encoding="utf-8")):
            offenders.append(path.relative_to(_ROOT).as_posix())
    assert offenders == [], offenders
    # The M10 field must not exist on Ticket yet (the real boundary, not a comment).
    assert "pii_redacted" not in Ticket.model_fields
...
# ===========================================================================
# The LIVE tests (opt-in) — budget: up to 6 calls total (2 M2/M3 + 2 M4 + 1 M5 + 1 M6)
# ===========================================================================
@pytest.mark.live
@pytest.mark.skipif(
