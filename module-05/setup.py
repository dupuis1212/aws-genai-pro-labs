"""setup.py — stand up Relay's managed Knowledge Base `relay-kb` (Module 5).

Module 5 of AWS GenAI Pro Mastery. Idempotent and verbose: safe to run twice, and
...
def ensure_knowledge_base(agent, s3v, role_arn: str, vector_bucket: str,
    ...
    created = agent.create_knowledge_base(
        ...
        storageConfiguration={
            ...
            "s3VectorsConfiguration": {
                ...
                # name should not be present with namespace arn") — the ARN is the
                # namespace, so we address the index by ARN only. (Pass indexName
                # INSTEAD of the ARN only when you let the KB create the index.)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Setting up Module 5 — Relay's managed Knowledge Base 'relay-kb'.")
    ...
    print("Expected cost: a few cents (one Titan ingestion job); KB idle ~$0.\n")
