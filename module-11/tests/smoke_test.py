"""smoke_test.py — Module 11 lab tests (cumulative: Modules 2–11).

Module 11 (serverless front door) tests are at the END of the file:
...
def test_no_M12_or_later_capabilities_in_lab_code():
    """Module 11 BUILDS the serverless front door — the public API / API Gateway / SQS /
    EventBridge `relay-events` bus / CDK / CodePipeline are its OWN increment, so those
    tokens are now EXPECTED (the brief §10 grep gate shifts to the M11 boundary). What M11
    must NOT USE is a DOWNSTREAM capability: no semantic cache (M12 — relay.cache), no eval
    harness / eval-gate WIRED in the pipeline (M13). Those, plus the forbidden agent tooling
    (legacy starter-toolkit, Bedrock Agents classic create_agent/invoke_agent, LangChain)
    and the banned legacy invoke path, are the forbidden TOKENS in the lab CODE.

    `relay-events` / `api_gateway` / `eventbridge` are ALLOWED at M11 (the increment
    itself). Scope = the M11 lab code (relay/, mcp_server/, cdk/, pipeline/, setup.py,
    teardown.py, audit_report.py). The eval-gate stage name appears ONLY in a COMMENT in
    pipeline_stack.py (the documented M13 boundary), never wired — asserted separately.
    """
    # The M12 capability is the relay.cache MODULE (the semantic cache); guard the import/
    # path, not the word "semantic cache" — inherited config.py prose names M12's cache in
    # a forward-pointer comment, which is documentation, not usage.
    pattern = re.compile(
        r"starter.toolkit|create_agent|invoke_agent|invoke_model|"
        r"langgraph|langchain|"
        r"relay/cache\b|relay\.cache\b|import cache",
        re.IGNORECASE,
    )
    ...
    # (the real forward-dependency boundary — M11 adds no schema field).
...
# OpenAPI client integrates against. At Module 11 it carries EXACTLY the three frozen
# routes; `/feedback` is added at Module 13 (06 §2 / bible §3.3).
...
def test_openapi_export_has_exactly_the_three_m11_routes_no_feedback():
    """The Module 11 export carries EXACTLY the three frozen routes — POST /tickets,
    GET /tickets/{ticket_id}, POST /tickets/{ticket_id}/approve — and NO /feedback
    (that route is added at Module 13). It matches the CDK WIRING SPEC field-for-field."""
    spec = _load_openapi()
    routes = _openapi_routes(spec)

    assert routes == EXPECTED_OPENAPI_ROUTES, routes
    assert len(routes) == 3
    # /feedback must NOT be in the M11 contract — it is added by addition at M13.
