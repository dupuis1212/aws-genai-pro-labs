...
Why this layering holds the contracts (bible §4 M11):
  ...
  - `TicketRecord` gains NO field — Module 11 only EXERCISES the full frozen status
    enum end-to-end (received -> triaged/awaiting_approval/answered/escalated/closed/
    failed). `POST /feedback` + `feedback_rating` are Module 13, not here.
