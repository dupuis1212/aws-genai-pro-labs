...
|---|---|
| `RelayApiStack` | API Gateway REST API (`POST /tickets`, `GET /tickets/{ticket_id}`, `POST /tickets/{ticket_id}/approve`), the four Lambda handlers (`post`/`get`/`approve`/`worker`), the SQS work queue + DLQ, and the `relay-events` EventBridge bus with a demo escalation rule. |
| `RelayPipelineStack` | The CodePipeline (`source → build → deploy → smoke → rollback`). The **eval-gate** stage is commented for Module 13. |
