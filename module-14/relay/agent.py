...
Module 10 adds — BY ADDITION below the "Module 10 ADDITIONS" banner, the M7/M8 surface
byte-identical — a structured DECISION LOG: handle()/handle_with_handoff() append one
JSON-Lines record per ticket (the tool calls, their REDACTED inputs, the result, the
outcome) to decision_log.jsonl. It is the application-level "what did the bot decide and
why?" trail the auditor asks for — the complement to CloudTrail's "who called which API"
(audit_report.py reads both). Inputs are re-redacted through relay.pii, so no raw PII
ever lands in the log.
...
from relay import config, pii, tools
...
def handle(
    ...
    *,
    ...
    # Module 10: append the structured, REDACTED decision-log record (the "why" trail
    # CloudTrail does not carry). Best-effort governance side-channel — a log-write
    # failure must not fail a handled ticket, so it is reported, never raised.
    if log_decision:
        try:
            write_decision_log(typed, stop_reason=stop_reason, path=decision_log_path)
        except OSError as err:
            print(f"[warn] decision log not written: {err}", file=sys.stderr)
...
def handle_with_handoff(
    ...
    *,
    ...
    # Module 10: the decision log captures the handoff + gate decisions too (the
    # auditor wants "did a human approve this refund?"). Inputs re-redacted; best-effort.
    if log_decision:
        try:
            write_decision_log(typed, handed_off=handed_off, gated=gated,
                               stop_reason=stop_reason, path=decision_log_path)
        except OSError as err:
            print(f"[warn] decision log not written: {err}", file=sys.stderr)
...
# =============================================================================
# Module 10 ADDITIONS — the structured DECISION LOG (governance, skills 3.3.1/3.3.2).
# =============================================================================
# Everything ABOVE the "Module 8 ADDITIONS" banner is the byte-identical Module 7 agent;
# the handoff/HITL block is Module 8. Module 10 adds — BY ADDITION, no rewrite of the M7/
# M8 surface — a DECISION LOG: one structured JSON-Lines record per handled ticket,
# written to decision_log.jsonl, that answers the auditor's "what did the bot DECIDE and
# why?" question. It is the COMPLEMENT to CloudTrail, and the exam draws the line:
#
#   - CloudTrail        : who called which AWS API, and when (read by audit_report.py).
#                         It records the API call, NOT the prompt content.
#   - this decision log : what RELAY decided — which tools it ran, on what (REDACTED)
#                         input, the result, the final status — the application-level
#                         "why". This is where prompt-shaped content would live, so it
#                         MUST be written redacted.
#
# PII DISCIPLINE: the inputs recorded here are RE-REDACTED through relay.pii before they
# are written, even though intake already masked the ticket — defense in depth, because a
# tool input could carry a value the agent assembled itself. NO raw email/name/phone ever
# lands in decision_log.jsonl (brief §10 — the smoke test asserts no clear email in it).


def _decision_log_path() -> Path:
    """The decision-log file (repo root / decision_log.jsonl) — git-ignored runtime data."""
    return Path(__file__).resolve().parent.parent / config.DECISION_LOG_FILE_NAME


def _redact_value(value) -> object:
    """Redact any PII in a logged value WITHOUT calling Comprehend.

    The decision log must never block an agent run on a network call, so this uses
    relay.pii's pure offset masker over a CHEAP local detector is NOT available here —
    instead we keep it conservative: we only pass through values that are already
    structural (ids, numbers, bools) and mask the string ones with the masking primitive
    when spans are supplied. In practice the inputs were already redacted at intake; this
    is the belt-and-braces pass. Recurses into dicts/lists so a nested tool input is
    covered. A non-string scalar (int/bool/None) is returned as-is."""
...
def _redact_string(text: str) -> str:
    """Mask obvious PII shapes in a free-text string for the decision log, offline.

    The decision log is written on every run and must not depend on a Comprehend call, so
    this is a SMALL, explicit local guard (email + a long digit run that looks like a
    phone/card) layered on top of intake's authoritative Comprehend redaction. It exists
    to catch a value the agent itself assembled post-intake — it is NOT the primary
    redactor (relay.pii is). Order numbers (`#1042`) are deliberately preserved: they are
    business keys, not PII, and the audit trail needs them to be useful."""
    masked = _EMAIL_RE.sub(pii.placeholder_for("EMAIL"), text)
    masked = _PHONE_RE.sub(_mask_if_phone_shaped, masked)
    return masked


def _mask_if_phone_shaped(match) -> str:
    """Mask a candidate digit run only if it is really phone/card shaped.

    A candidate qualifies as a PHONE only when it carries a real formatting signal — a
    leading '+' or at least one separator (space/dash/paren) — AND has 7+ digits. A bare
    run of glued digits (the 12-digit AWS account id in `relay-901353600690`, an order
    number) has no separators and is LEFT INTACT so the audit trail's source URIs stay
    readable.

    Two NON-phone shapes are explicitly preserved, because the module's whole teaching is
    that operational signal and business keys survive redaction (the article, the model
    card, and config.PII_ENTITY_TYPES all promise it): an ISO-8601 date (`2026-06-10`, a
    delivery/billing date the agent needs) and a hyphen-grouped reference code (a tracking
    number such as `7741-5521-9080`, blocks of four digits). Neither is a phone number, so
    masking them to `[PHONE]` would make the decision log contradict the design. A real
    phone (`555-0100`, `1-800-555-0100`, `212) 555-0199`) does not fit either shape and is
    still masked. Comprehend at intake is the authoritative phone redactor; this is the
    offline belt-and-braces for a value the agent re-assembled post-intake."""
    candidate = match.group(0).strip()
    if _ISO_DATE_RE.fullmatch(candidate) or _GROUPED_CODE_RE.fullmatch(candidate):
        return match.group(0)            # a date / tracking code — operational, not PII
    digits = sum(c.isdigit() for c in candidate)
    has_signal = candidate.startswith("+") or any(c in " ().-" for c in candidate)
    if digits >= 7 and has_signal:
        return pii.placeholder_for("PHONE")
    return match.group(0)


# A deliberately narrow email/phone guard for the LOG ONLY (intake's Comprehend pass is
# the authoritative redactor; this is the offline belt-and-braces over self-assembled
# values). Kept local to the log path so it never competes with relay.pii at the edge.
import re as _re  # noqa: E402  (local to the M10 decision-log block)
_EMAIL_RE = _re.compile(r"[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}")
# Candidate phone/card-SHAPED runs: an optional '+' then a run of digits and phone
# separators (space/dash/paren). The regex is deliberately broad — _mask_if_phone_shaped
# is the real gate: it masks ONLY a candidate that has 7+ digits AND a real formatting
# signal (a '+' or a separator), so the glued 12-digit AWS account id in the canonical
# bucket name `relay-901353600690` (no separators) is left intact and source URIs in the
# audit trail stay readable. Comprehend at intake is the authoritative phone redactor.
_PHONE_RE = _re.compile(r"\+?\d[\d\s().\-]{5,}\d")
# An ISO-8601 calendar date (YYYY-MM-DD): a delivery/billing date is operational signal
# the agent needs (config.PII_ENTITY_TYPES deliberately does NOT mask DATE_TIME), so the
# log guard must leave it intact and never confuse it with a phone number.
_ISO_DATE_RE = _re.compile(r"\d{4}-\d{2}-\d{2}")
# A hyphen-grouped reference code: two or more blocks of FOUR-OR-MORE digits joined by
# single hyphens (a tracking number such as `7741-5521-9080` or `1042-88213`). It is a
# business key, not PII, and the audit trail needs it readable; a US phone (`555-0100`,
# 3-4 grouping; `1-800-555-0100`, 1-3-3-4) never opens with a 4+-digit block followed by
# another 4+-digit block, so it does not match and is still masked.
_GROUPED_CODE_RE = _re.compile(r"\d{4,}(?:-\d{4,})+")


def _action_log_entry(action: AgentAction) -> dict:
    """One auditable, REDACTED record of a single tool call for the decision log."""
    return {
        "tool": action.tool,
        "tool_input": _redact_value(action.tool_input),
        "result": _redact_string(action.result),
...
def write_decision_log(
    ...
    *,
    ...
    """Append one structured decision record for a handled ticket (skills 3.3.1/3.3.2).

    Writes a single JSON object (one line) capturing WHAT the agent decided: the ticket
    id, the final status, whether it handed off / gated a sensitive action, the stop
    reason, and the REDACTED action journal (tool, masked input, result, approved). The
    inputs are re-redacted (defense in depth) so NO raw email/phone reaches the file.
    Returns the dict it wrote (so a caller/test can assert on it without re-reading the
    file). Appends in JSON-Lines so the log is greppable and audit_report.py can stream
    it. This is the application "why" trail — CloudTrail is the AWS-API "who" trail."""
    entry = {
        "ts": _now_iso(),
        ...
        "actions": [_action_log_entry(a) for a in record.actions],
    }
    target = path or _decision_log_path()
