"""smoke_test.py — Module 12 lab tests (cumulative: Modules 2–12).

Module 12 (the token economy) tests are at the VERY END of the file (after the M11 ones):
...
def test_no_M13_or_later_capabilities_in_lab_code():
    """Module 12 BUILDS the cost instrumentation + the four optimization levers — the
    semantic cache (relay.cache), prompt caching + the Flex tier on converse() (via
    **params), batch inference, and the populated TicketRecord.cost_cents are its OWN
    increment, so `relay.cache` / `cache_prompt` / `service_tier` / batch tokens are now
    EXPECTED. What M12 must NOT USE is a DOWNSTREAM capability: no eval harness / golden set
    / LLM-as-a-judge / regression gate WIRED (M13 — M12 provides only the batch path), and
    no `/feedback` endpoint or `feedback_rating` schema field (M13). Those, plus the
    forbidden agent tooling (legacy starter-toolkit, Bedrock Agents classic create_agent/
    invoke_agent, LangChain) and the banned legacy invoke path, are the forbidden TOKENS in
    the lab CODE.

    The M11 front door (`relay-events`, API Gateway, SQS, CDK, CodePipeline) and the M12
    levers (`relay.cache`, `cache_prompt`, `service_tier`, batch) are ALLOWED — they are the
    built increments. Scope = the lab code (relay/, mcp_server/, cdk/, pipeline/, setup.py,
    teardown.py, audit_report.py, cost_report.py). The eval-gate stage name still appears
    ONLY in a COMMENT in pipeline_stack.py (the documented M13 boundary), never wired.
    """
    # The forbidden DOWNSTREAM (M13+) capabilities. relay.cache / cache_prompt / service_tier
    # are M12's own build, so they are NOT here. The legacy invoke path is still banned
    # everywhere but ingest/embed.py (Titan embeddings — the course's sole exception).
    # NOTE: `feedback_rating` / `/feedback` are NOT in this regex — inherited docstrings
    # name them as the documented M13 boundary (forward-pointers, not usage). The REAL
    # boundary (no such schema field, no wired route) is asserted structurally below.
    pattern = re.compile(
        r"starter.toolkit|create_agent|invoke_agent|invoke_model|"
        r"langgraph|langchain|"
        r"import evals|from evals|run_evals|golden_set|llm_judge",
        re.IGNORECASE,
    )
    ...
    assert offenders == [], offenders
    # The M10 Ticket field still exists; the M13 feedback_rating must STILL be absent
    # (the real forward-dependency boundary — M12 adds no schema field).
...
def test_m8_adds_no_new_schema_and_keeps_models_frozen():
    ...
    assert "feedback_rating" not in TicketRecord.model_fields  # M13 boundary
    # M8 itself added no Ticket field; the only Ticket addition by now is M10's
    # pii_redacted (AgentAction / TicketRecord above stay byte-identical to M7).
...
def test_m9_adds_no_new_schema_models_byte_identical():
    ...
    assert set(Answer.model_fields) == {"text", "citations", "grounded"}
    # M9 added no field; M10 then adds exactly one (Ticket.pii_redacted). The schema
    # SET above is still {7 classes} — M10 adds a field, not a class.
    assert "pii_redacted" in Ticket.model_fields                 # added at M10
    assert "feedback_rating" not in TicketRecord.model_fields    # M13 boundary
...
def test_m11_config_did_not_touch_the_tier_map_or_embedder_or_schemas():
    """M11 appends only resource NAMES — no new model, no re-point, no schema field."""
    assert config.tier_profile("fast") == "us.amazon.nova-micro-v1:0"
    assert config.tier_profile("smart") == "us.amazon.nova-2-lite-v1:0"
    assert config.EMBED_MODEL_ID == "amazon.titan-embed-text-v2:0"
    assert set(config.TIERS) == {"fast", "smart", "frontier", "vision"}  # unchanged
    # No Pydantic field added anywhere at M11 (the full status enum is only EXERCISED).
    assert "feedback_rating" not in TicketRecord.model_fields   # M13 boundary still holds
    assert list(TicketRecord.model_fields) == [
        "ticket_id", "status", "triage", "answer", "actions", "escalated",
        "cost_cents", "updated_at",
    ]
...
def test_api_routes_are_the_frozen_contract_field_for_field():
    ...
    GET /tickets/{ticket_id}, POST /tickets/{ticket_id}/approve."""
    ...
    # The four handlers point at relay.api.<module>.lambda_handler (the WRAPPED agent).
    assert set(api_stack.LAMBDA_HANDLERS) == {"post", "get", "approve", "worker"}
...
# OpenAPI client integrates against. At Module 12 it carries EXACTLY the same three
# frozen routes as Module 11 (byte-identical export); `/feedback` is added at Module
# 13 (06 §2 / bible §3.3).
...
def test_openapi_export_has_exactly_the_three_m12_routes_no_feedback():
    """Module 12 inherits the M11 export byte-identical — EXACTLY the three frozen routes
    (POST /tickets, GET /tickets/{ticket_id}, POST /tickets/{ticket_id}/approve) and NO
    /feedback (that route is added at Module 13). It matches the CDK WIRING SPEC."""
    spec = _load_openapi()
    routes = _openapi_routes(spec)

    assert routes == EXPECTED_OPENAPI_ROUTES, routes
    assert len(routes) == 3
    # /feedback must NOT be in the M12 contract — it is added by addition at M13.
    assert not any("/feedback" in path for _method, path in routes)
...
# --- the pipeline stage order + the eval-gate is COMMENTED (M13 boundary) -----
def test_pipeline_stages_are_build_smoke_deploy_with_eval_gate_for_m13():
    from cdk.relay_cdk import pipeline_stack

    assert pipeline_stack.PIPELINE_STAGES == ("Source", "Build", "Deploy", "Smoke")
    # The eval-gate is the M13 addition — it must NOT be a wired stage at M11.
    assert "EvalGate" not in pipeline_stack.PIPELINE_STAGES
    assert pipeline_stack.EVAL_GATE_STAGE_M13 == "EvalGate"
    # It appears ONLY in a COMMENT in pipeline_stack.py (documented forward boundary),
    # never as an executed StageProps.
    src = (_ROOT / "cdk" / "relay_cdk" / "pipeline_stack.py").read_text("utf-8")
    for line in src.splitlines():
        if "EvalGate" in line and "StageProps" in line:
            assert line.lstrip().startswith("#"), (
                "the EvalGate StageProps must stay COMMENTED until Module 13")
...
def test_m12_did_not_touch_the_tier_map_or_embedder_or_schemas():
    """M12 is BY ADDITION: the M3 tier map, the Titan embedder, and the frozen schemas are
    byte-identical (the price map already existed since M3 — M12 only CONSUMES it)."""
    assert config.TIERS == {
    ...
    # cost_cents exists (M7), feedback_rating still absent (M13 boundary).
