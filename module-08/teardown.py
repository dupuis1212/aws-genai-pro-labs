"""teardown.py — purge AgentCore Memory + remove M7's MCP Lambda, KEEP downstream needs.

Module 8 of AWS GenAI Pro Mastery. Idempotent and verbose, and TESTED — one tested
teardown per setup (decision B5).

What this DELETES BY DEFAULT (the idle-billed / standing resources):
  0. (Module 8) the **AgentCore Memory** store `relay-memory` — its long-term
...
def assert_clean(*, drop_tables: bool, drop_kb: bool, drop_vectors: bool,
    ...
    print("To rebuild: uv run python setup.py "
          "(recreates the MCP Lambda + AgentCore Memory; tables/KB kept).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Tearing down Module 8 (idempotent).\n")
    ...
    try:
        print("AgentCore Memory (Bedrock AgentCore — the only idle-billed item):")
