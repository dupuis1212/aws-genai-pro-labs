"""smoke_test.py — Module 8 lab tests (cumulative: Modules 2, 3, 4, 5, 6, 7 and 8).
...
LIVE-CALL BUDGET: at most EIGHT calls total —
...
Together that is well under $0.06 as of June 2026. They need AWS credentials and
us-east-1. NO live test creates or deletes a KB / Lambda / table / AgentCore Memory,
and NONE uploads to S3 (the live vision call reads the screenshot from local bytes).
"""
...
def test_no_M9_or_later_capabilities_in_lab_code():
    """M8 builds the handoff, AgentCore (Runtime/Memory), and the HITL approval flow
    (its own increment — those tokens are now EXPECTED). But it must NOT USE a downstream
    capability: no guardrail layer / ApplyGuardrail (M9), no PII redaction pipeline (M10),
    no public API / API Gateway / EventBridge `relay-events` bus (M11), no semantic cache
    (M12), no eval harness (M13). Those, plus the forbidden agent tooling (legacy
    starter-toolkit, Bedrock Agents classic create_agent/invoke_agent, LangChain), are
    forbidden TOKENS in the lab CODE. (brief §10 grep gate)

    `agentcore` / `bedrock_agentcore` are ALLOWED at M8 (the runtime SDK + CLI config);
    the forbidden list is exactly the legacy/out-of-scope tokens. Scope = the M8 lab
    code (relay/, mcp_server/, setup.py, teardown.py) — inherited helper scripts carry
    their own benign prose (freshness_test.py mentions an EventBridge SCHEDULE for doc
    re-sync, unrelated to the M11 bus) and are byte-identical from upstream.
    """
    pattern = re.compile(
        r"starter.toolkit|create_agent|invoke_agent|invoke_model|"
        r"langgraph|langchain|relay-events|api[_-]?gateway|eventbridge|"
        r"apply_guardrail|ApplyGuardrail|DetectPiiEntities|relay/cache|relay\.cache",
        re.IGNORECASE,
    )
    ...
    for path in lab_files:
        ...
        for m in pattern.finditer(text):
            ...
            # embeddings — the course's sole InvokeModel). It does not appear in relay/.
...
def test_live_handoff_proposes_a_refund_awaiting_approval():
    ...
    try:
        ...
        pytest.skip("MCP server not set up (run setup.py) — skipping live handoff run.")
