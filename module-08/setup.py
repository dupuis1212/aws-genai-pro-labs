"""setup.py — stand up the agent's resources + AgentCore Memory, on top of the KB (M8).

Module 8 of AWS GenAI Pro Mastery. Idempotent and verbose: safe to run twice, and
...
def main(argv: list[str] | None = None) -> int:
    ...
    known = ("--no-wait", "--skip-kb", "--skip-memory")
    ...
    print("Setting up Module 8 — deploy Relay on Bedrock AgentCore: the agent's tables "
          "+ MCP\nLambda (M7) AND the AgentCore Memory store (short + long term), on top "
          "of the KB.")
